...
import ParallaxImageSplit, {
  ParallaxImageTextSection,
} from "../../components/ParallaxImageSplit";
...
function ImageCarouselDots({ len, idx = 0 }: ImageCarouselDotsProps) {
  ...
  return <div className="ImageCarouselDots__carousel-dots">{dots}</div>;
}
...
function rollover(dir: "up" | "down", value: number, limit: number) {
  if (dir === "up") return ++value > limit ? 0 : value;
...
function ParallaxImagesSection({ items }: ParallaxImagesSectionProps) {
  ...
  return (
    <section id="parallax-images" className="landing-content-spacing">
      <div className="single-cell ParallaxImagesSection__wrapper">{result}</div>

      <div className="single-cell ParallaxImagesSection__bg"></div>
    </section>
  );
}
...
function ImageCarousel({ images, startIdx = 0 }: ImageCarouselProps) {
  ...
  return (
    <div className="ImageCarousel__image-carousel landing-content-spacing">
      <div
        role="img"
        className={`div-as-img ${switching ? "ImageCarousel__fade-out" : ""}`}
        style={{ backgroundImage: `url(${images[currentIdx].url})` }}
      ></div>
      {switching && (
        <div
          role="img"
          className={`div-as-img ${switching ? "ImageCarousel__fade-in" : ""}`}
          style={{
...
function HeroTransition() {
    ...
    return (
      <div id="transition" className="single-cell">
        ...
        >
          ...
          <path
            fill={TRANSITION_COLORS[1]}
            fillOpacity="1"
            d="M0,192L24,192C48,192,96,192,144,181.3C192,171,240,149,288,128C336,107,384,85,432,101.3C480,117,528,171,576,176C624,181,672,139,720,133.3C768,128,816,160,864,154.7C912,149,960,107,1008,117.3C1056,128,1104,192,1152,181.3C1200,171,1248,85,1296,80C1344,75,1392,149,1416,186.7L1440,224L1440,320L1416,320C1392,320,1344,320,1296,320C1248,320,1200,320,1152,320C1104,320,1056,320,1008,320C960,320,912,320,864,320C816,320,768,320,720,320C672,320,624,320,576,320C528,320,480,320,432,320C384,320,336,320,288,320C240,320,192,320,144,320C96,320,48,320,24,320L0,320Z"
          ></path>
        </svg>
        <svg
          style={{ marginTop: "-3px" }}
  ...
  return (
    <section id="theory" className="landing-content-spacing">
      <div id="theory-title">
        Remote peer mentoring to minimize CCUSD's socioeconomic achievement gap.{" "}
      </div>
      <div style={{ display: "grid", height: "100%" }}>
        <Transition />
        <div className="single-cell" id="theory-explanation">
          <p>
            One Step Ahead Culver City is a 501(c)3 non-profit organization and
            Culver City High School club working towards a more equitable future
            for all students in CCUSD. We connect students from all five
            elementary schools with high school mentors who assist them in their
            individual struggles with reading and math.
          </p>
          ...
          <p>
            Our vision is centered around community: we work with teachers, MTSS
            specialists, administrators, parents, and students to ensure our
            work is impactful and long-lasting. We hope to not only help our
            students catch up but to build intrinsic learning motivation and
            growth mindsets that will help them for the rest of their lives.
          </p>
        </div>
      </div>
    </section>
...
function MobileHero() {
  return (
    <div className="MobileHero__main">
      <div
        className="MobileHero__focus-image"
        style={{
...
function HeroSection() {
  ...
  return <section id="hero">{focus}</section>;
}
...
function SupportSection() {
  ...
  return (
    <section
      ref={sectionRef}
      id="supporters"
      className="landing-content-spacing"
    >
      <div className="Header__title">Thank You To Our Friends At</div>

      <div
        className={`SupportSection__images ${visible ? "SupportSection__fade-in" : ""}`}
      >
        <img
          data-do-not-render-mobile
          alt="Culver City Unified School District"
          style={{ maxWidth: "100px" }}
          src={ccusd}
        />
        <img
          alt="El Marino Language School"
          className="standard-image"
          src={elmarinologo}
        />
        <img
          alt="El Rincon Elementary School"
          className="standard-image"
          src={elrinconlogo}
        />
        <img
          alt="La Ballona Elementary School"
          className="standard-image"
          src={laballonalogo}
        />
        <img
          alt="Linwood E. Howe Elementary School"
          className="standard-image"
          src={linhowelogo}
        />
        <img
          alt="Farragut Elementary School"
          className="standard-image"
          src={farragutlogo}
        />
      </div>
      ...
      >
        <img
          alt="Culver City Education Foundation"
          className="standard-image"
          src={ccef}
        />
      </div>
...
const PARALLAX_IMAGES: ParallaxImageItem[] = [
  {
    title:
      "Students helping students one-on-one to stay one step ahead in their education",
    content: (
      <>
        <div>
          <p>
            A student's ability to perform in the classroom is heavily tied to
            factors outside of their control. Students from socioeconomically
            disadvantaged households are statistically more likely to fall
            behind and stay behind in both reading and math in our current
            school system.
          </p>
          <p>
            Our one-on-one approach to peer mentoring ensures that each
            student's individual needs are met to keep them academically
            motivated and engaged. We hope that through our work we can create a
            more diverse future generation of leaders in Culver City,
            California.
          </p>
        </div>
        <ButtonGroup>
  ...
  {
    title:
      "Our mentors are all passionate high school volunteers working to share their knowledge and skills",
    content: (
      <>
        <div>
          <p>
            Our mentors are all passionate high school volunteers working to
            share their knowledge and skills in hopes of a more equitable
            future.
          </p>
          <p>
            We require that our mentors be extremely well versed in their
            subject matter, and perform extensive training to ensure that every
            session is effective.
          </p>
          <p>
            Above all, our goal is for each student to consider their mentor a
            real friend.
          </p>
        </div>
        <ButtonGroup>
  ...
  {
    title:
      "We rely on the Culver City community for support to make our program the best it can be",
    content: (
      <>
        <div>
          <p>
            We gather input from teachers, specialists, and parents to target
            the students who need our help the most. One Step Ahead provides
            these kids with assistance that they may not have received otherwise
            in their families.
          </p>
          <p>
            While all our mentors and leaders double as volunteers, we still
            require funds to keep our program going. We are incredibly grateful
            to the community members and organizations who make it possible for
            us to buy books for students, fund the legal side of running a
            nonprofit, register a website domain, and cover extra program costs.
          </p>
        </div>
        <ButtonGroup>
          <Button
...
const Quotes: FC<PropsWithChildren> = (props) => {
  return <ul className="Quotes">{props.children}</ul>;
};
...
const Quote: FC<PropsWithChildren & { by: string }> = (props) => {
  return (
    <li className="Quote">
      ...
      <div className="Quote__author">&mdash; {props.by}</div>
    </li>
  );
};
...
export default function Landing() {
  ...
  return (
    <main id="Landing">
      ...
      <FounderBlurb
        left
        colors={[GlobalStyles.COLOR_MAIN, GlobalStyles.COLOR_SPLASH]}
        url={zoom}
        title="Testimonials"
        className="landing-content-spacing landing-content-v-spacing"
      >
        <div>
          ...
          <Quotes>
            <Quote by="Antonette">
              <div>
                I almost automatically saw a massive boost in Sebastian's
                confidence level when reading aloud with him or with me.
                Decoding words with Miles, he says, is really cool and fun to
                do! The way in which they read together over Zoom, paragraph by
                paragraph, and then discussing what the story is about... and
                knowing it's okay not to know the definition of certain words
                are two critical factors that I feel Miles is developing and
                showing Sebastian weekly!
              </div>
            </Quote>
          </Quotes>
        </div>
      ...
      <div
        ref={greeting}
        id="visit-us"
        className={`Header__title ${visible ? "Landing__fade-in" : ""}`}
      >
