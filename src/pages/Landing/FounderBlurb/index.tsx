...
function Circle({ background, radius, ...placement }: CircleProps) {
  return (
    <div
      className="Circle"
      style={{ background, height: radius, width: radius, ...placement }}
    ></div>
  );
}
...
export default function FounderBlurb({
  ...
  className,
  ...props
}: PropsWithChildren<FounderBlurbProps>) {
  // we need to add spacing to the front if we append it
  if (!!className) className = " " + className;

  return (
    <section
      className={"FounderBlurb" + (className ?? "")}
      {...(left ? { "data-left": true } : {})}
    >
      <div
        className="FounderBlurb__text-section"
        {...(left ? { style: { order: 1 } } : {})}
      >
        <h2>{title}</h2>
